...
| `-t, --lang LANG` | — | only these languages, comma-separated |
| `--no-tests` | off | skip files that name themselves tests |
| `--tests-only` | off | only files that name themselves tests |
...
| `-O, --orphan-context N` | 6 | lines either side of a match no declaration encloses |
| **Budget** | | |
| `-b, --max-tokens N` | 2000 | budget for the whole result set; no default under `--vimgrep` |
...
| `--why` | off | show what each span's score is made of |
| `--json` | off | emit JSON |
...
```console
rkgrep -e validate_token -e Claims -e refresh src/ -b 4000
```
...
```console
$ rkgrep -e Claims -e refresh src -b 400 -l
...
`-b` are mutually exclusive, so a run can never claim both.

`-O, --orphan-context N` sets how many lines come back either side of a match
no declaration encloses — imports, top-level constants, configuration literals
— and how far into a declaration too large to return the window reaches. A file
with no declarations in it at all is nothing but these windows, which is the
case `rg -C` already serves.
...
`-t py,rust` keeps only the files those languages are written in, decided by
extension before a file is opened. A name that is not a language is refused and
the error names every language that is accepted, because an empty result set
and a misspelled language look the same otherwise. `-t` and `-g` narrow
independently: both have to admit a file for it to be searched.

`--no-tests` drops the files that name themselves tests and `--tests-only`
returns nothing else. A test is a file under a `test`, `tests`, `spec`, `specs`,
`testing` or `__tests__` directory, or one named like `auth_test.go`,
`test_auth.py`, `api.test.ts`, `user_spec.rb` or `AuthTest.java`. The path is
all that is read, so `latest.py` is not one.
...
back: `rkgrep -l TODO src | rkgrep --fetch - src`. `--json` and `-b` apply, and
...
```console
$ rkgrep -w declaration_name src -b 200
...
- **No budget.** Enumerating matches and filling a context window are different
  questions, and a budget would answer the second one silently. `-b` still
...
`--why` writes what a span's score is made of on its own line under the header,
so a ranking that looks wrong can be read rather than guessed at:

```console
$ rkgrep --why -d -w declaration_name src
spans/scan.rs:296-300 (fn declaration_name) [45 tok]
  why matches 0.69 + terms 1.50 + path 0.00 - depth 0.00 = 2.19, declaration
pub fn declaration_name(line: &str) -> Option<&str> {
...
```

The four parts are the whole of the score: how many lines matched, how much of
the pattern the span's text carries, how much the file's own name looks like
the pattern, and what a shallower declaration of the same name costs this one.
The weights behind them are in `search/rank.rs`. `declaration` at the end is
not part of the sum — declarations sort ahead of everything else whatever they
score, which the number alone would not explain. See
...
| `why` | the four parts that sum to `score`; present only under `--why` |
...
| `0` | something matched |
| `1` | nothing matched |
| `2` | bad pattern, bad glob, bad anchor, an anchor outside the root, an unknown `--lang`, or a path that does not exist |
